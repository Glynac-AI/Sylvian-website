...
export default function CapitalStackVisualization({ capitalStack }: CapitalStackVisualizationProps) {
    return (
        ...
        >
            {/* UPDATED: Heading to 3xl */}
            <h2 className="text-3xl font-semibold text-[#095520] mb-6">Capital Stack Structure</h2>

            {/* UPDATED: Body to text-base */}
            ...
            <div className="space-y-4">
                {capitalStack.map((layer, index) => (
                    <motion.div
                        ...
                        className="relative"
                    >
                        <div className="flex items-center justify-between p-5 rounded-xl border-2 border-gray-100 hover:border-[#095520]/20 transition-all duration-300">
                            <div className="flex items-center gap-6 flex-1">
                                <div className="flex-shrink-0 w-20 text-center">
                                    {/* UPDATED: Percentage to 3xl */}
                                    <p className="text-3xl font-semibold text-[#095520]">{layer.percentage}%</p>
                                </div>
                            ...
                            <div className="flex-shrink-0 text-right ml-4">
                                {/* UPDATED: Amount to text-xl */}
                                <p className="text-xl font-semibold text-black">{layer.amount}</p>
                                {layer.yieldRange && (
                                    <p className="text-sm text-[#095520]/70 mt-1">
                                        {layer.yieldRange}
                                    </p>
                                )}
                            </div>
