...
interface NormalizedSponsor {
    ...
    operatingRecord: {
        ...
        disclosureBehavior: { transparencyRating: 'TOP TIER' | 'STRONG' | 'ADEQUATE' }
    }
    team: { name: string; role: string; experience: string; bio: string }[]
...
export default function SponsorsClient({ sponsors }: SponsorsClientProps) {
    const [selectedCompliance, setSelectedCompliance] = useState('All')

    const filteredSponsors = sponsors.filter(sponsor => {
        return selectedCompliance === 'All' || sponsor.complianceStatus === selectedCompliance
    })
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="flex flex-wrap items-center gap-4">
                        ...
                        <div className="flex gap-2">
                            {(['All', 'EXCELLENT', 'GOOD'] as const).map((status) => (
                                <button
                                    key={status}
                                    onClick={() => setSelectedCompliance(status)}
                                ...
                                >
                                    {status === 'All' ? 'All Sponsors' : status.charAt(0) + status.slice(1).toLowerCase()}
                                </button>
                            ))}
                        </div>
                    ...
                    {filteredSponsors.length > 0 ? (
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {filteredSponsors.map((sponsor, index) => (
                                <SponsorCard key={sponsor.id} sponsor={sponsor as any} index={index} />
                            ))}
                        </div>
                    ) : (
                        <motion.div
